...
test("renders chat interface with message list and input", () => {
  ...
  // No messages → shows empty state instead of message-list
...
test("scrolls when messages change", () => {
  ...
  const scrollContainer = screen.getByTestId("message-list-empty-state").closest("[data-radix-scroll-area-viewport]");
...
test("renders with correct layout classes", () => {
  ...
  const scrollArea = screen.getByTestId("message-list-empty-state").closest(".flex-1");
